...
static void igb_create_lbtest_frame(struct m_buf *mbuf,
                                    unsigned int frame_size)
{
	memset(mbuf->data, 0xFF, frame_size);
	frame_size /= 2;
	memset(&mbuf->data[frame_size], 0xAA, frame_size - 1);
	memset(&mbuf->data[frame_size + 10], 0xBE, 1);
	memset(&mbuf->data[frame_size + 12], 0xAF, 1);
}

static int igb_check_lbtest_frame(struct m_buf *mbuf, unsigned int frame_size)
{
	frame_size /= 2;
	if (*(mbuf->data + 3) == 0xFF) {
		if ((*(mbuf->data + frame_size + 10) == 0xBE) &&
		   (*(mbuf->data + frame_size + 12) == 0xAF)) {
			return 0;
		}
	}
	return 13;
                                ...
                                unsigned int size)
{
	...
	while (staterr & E1000_RXD_STAT_DD) {
		...
		/* verify contents of mbuf */
		if (!igb_check_lbtest_frame(buffer_info->mbuf, size))
			count++;
...
static int igb_run_loopback_test(struct igb_adapter *adapter)
{
	...
	/* place data into test mbuf */
	igb_create_lbtest_frame(mbuf, size);
	mbuf_put(mbuf, size);
	...
	for (j = 0; j <= lc; j++) { /* loop count loop */
		...
		for (i = 0; i < 64; i++) {
			tx_ret_val = igb_xmit_frame_ring_adv(mbuf, tx_ring);
