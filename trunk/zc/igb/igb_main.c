...
#define DRV_VERSION "2.1.9" VERSION_SUFFIX DRV_DEBUG DRV_HW_PERF
...
static netdev_tx_t igb_xmit_frame_adv(struct m_buf *mbuf, struct net_device *);
static netdev_tx_t igb_xmit_frame_adv_fake(struct sk_buff *skb, struct net_device *netdev);
...
static const struct net_device_ops igb_netdev_ops = {
	.ndo_open		= igb_open,
	.ndo_stop		= igb_close,
	.ndo_start_xmit		= igb_xmit_frame_adv_fake,
...
#endif
	netdev->hard_start_xmit = &igb_xmit_frame_adv_fake;
#endif /* HAVE_NET_DEVICE_OPS */
                                    ...
                                    struct igb_buffer *buffer_info)
{
	...
	if (buffer_info->mbuf) {
		nta_kfree_mbuf(buffer_info->mbuf);
		buffer_info->mbuf = NULL;
	}
...
static inline int igb_tx_map_adv(struct igb_ring *tx_ring, struct m_buf *mbuf,
                                 unsigned int first)
{
	struct igb_buffer *buffer_info;
	struct pci_dev *pdev = tx_ring->pdev;
	unsigned int len = mbuf_headlen(mbuf);
	...
	buffer_info->dma = pci_map_single(pdev, mbuf->data, len,
	                                  PCI_DMA_TODEVICE);
	...
	tx_ring->buffer_info[i].mbuf = mbuf;
	tx_ring->buffer_info[i].gso_segs = mbuf_shinfo(mbuf)->gso_segs ?: 1;
...
netdev_tx_t igb_xmit_frame_ring_adv(struct m_buf *mbuf,
                                    struct igb_ring *tx_ring)
{
	...
	u8 hdr_len = 0;
#ifdef SIOCSHWTSTAMP
	union mbuf_shared_tx *shtx = mbuf_tx(mbuf);
#endif

	/* need: 1 descriptor per page,
	 *       + 2 desc gap to keep tail from touching head,
	 *       + 1 desc for mbuf->data,
	 *       + 1 desc for context descriptor,
	 * otherwise try next time */
	if (igb_maybe_stop_tx(tx_ring, mbuf_shinfo(mbuf)->nr_frags + 4)) {
...
#endif
	if (vlan_tx_tag_present(mbuf) && adapter->vlgrp) {
		tx_flags |= IGB_TX_FLAGS_VLAN;
		tx_flags |= (vlan_tx_tag_get(mbuf) << IGB_TX_FLAGS_VLAN_SHIFT);
	}
...
#ifdef NETIF_F_TSO
	if (mbuf_is_gso(mbuf)) {
		tso = igb_tso_adv(tx_ring, mbuf, tx_flags, &hdr_len);
	...
	else if (igb_tx_csum_adv(tx_ring, mbuf, tx_flags) &&
	 ...
	 */
	count = igb_tx_map_adv(tx_ring, mbuf, first);
	if (count <= 0) {
		nta_kfree_mbuf(mbuf);
	...
	igb_tx_queue_adv(tx_ring, tx_flags, count, mbuf->len, hdr_len);
...
static netdev_tx_t igb_xmit_frame_adv(struct m_buf *mbuf,
                                      struct net_device *netdev)
{
	struct igb_adapter *adapter = netdev_priv(netdev);
#ifdef HAVE_TX_MQ
	int r_idx = mbuf->queue_mapping & (IGB_MAX_TX_QUEUES - 1);
#endif
	//printk("xmit: mbuf->len %d mbuf->dir %d idx %d\n", 
	//	   mbuf->len, mbuf->dir, r_idx);
...
#ifdef HAVE_TX_MQ
	return igb_xmit_frame_ring_adv(mbuf, adapter->multi_tx_table[r_idx]);
#else
	return igb_xmit_frame_ring_adv(mbuf, adapter->tx_ring[0]);
#endif
}

static netdev_tx_t igb_xmit_frame_adv_fake(struct sk_buff *skb,
                                      struct net_device *netdev)
{
	kfree_skb(skb);
	return NETDEV_TX_OK;
...
#define MAX_STD_JUMBO_FRAME_SIZE 9234
	...
	/* NOTE: netdev_alloc_mbuf reserves 16 bytes, and typically NET_IP_ALIGN
...
static void igb_systim_to_hwtstamp(struct igb_adapter *adapter,
                                   struct mbuf_shared_hwtstamps *shhwtstamps,
                                   u64 regval)
{
	u64 ns;
	...
	memset(shhwtstamps, 0, sizeof(struct mbuf_shared_hwtstamps));
 ...
 * @mbuf: packet that was just sent
 *
 * If we were asked to do hardware stamping and such a time stamp is
 * available, then it must have been for this mbuf here because we only
 * allow only one such packet into the queue.
 */
static void igb_tx_hwtstamp(struct igb_q_vector *q_vector, struct m_buf *mbuf)
{
	struct igb_adapter *adapter = q_vector->adapter;
	union mbuf_shared_tx *shtx = mbuf_tx(mbuf);
	struct e1000_hw *hw = &adapter->hw;
	...
	/* if mbuf does not support hw timestamp or TX stamp not valid exit */
	...
	mbuf_tstamp_tx(mbuf, &shhwtstamps);
}
	...
	while ((eop_desc->wb.status & cpu_to_le32(E1000_TXD_STAT_DD)) &&
		...
		for (cleaned = false; !cleaned; count++) {
			...
			if (mbuf) {
				...
				bytecount = ((segs - 1) * mbuf_headlen(mbuf)) +
				            mbuf->len;
				total_packets += segs;
...
/**
 * igb_receive_mbuf - helper function to handle rx indications
 * @q_vector: structure containing interrupt and ring information
 * @mbuf: packet to send up
 * @vlan_tag: vlan tag for packet
 **/
static void igb_receive_mbuf(struct igb_q_vector *q_vector,
                            struct m_buf *mbuf,
                            u16 vlan_tag)
{
	struct igb_adapter *adapter = q_vector->adapter;
...
static inline void igb_rx_checksum_adv(struct igb_ring *ring,
                                       u32 status_err, struct m_buf *mbuf)
{
                                   ...
                                   struct m_buf *mbuf)
{
	 ...
	 *
	 * If nothing went wrong, then it should have a mbuf_shared_tx that we
	 * can turn into a mbuf_shared_hwtstamps.
	 */
	if (staterr & E1000_RXDADV_STAT_TSIP) {
		u32 *stamp = (u32 *)mbuf->data;
		regval = le32_to_cpu(*(stamp + 2));
		regval |= (u64)le32_to_cpu(*(stamp + 3)) << 32;
		mbuf_pull(mbuf, IGB_TS_HDR_LEN);
	} else {
	...
	igb_systim_to_hwtstamp(adapter, mbuf_hwtstamps(mbuf), regval);
}
#endif
#ifndef CONFIG_IGB_DISABLE_PACKET_SPLIT
 ...
 **/
static inline struct m_buf *igb_transform_rsc_queue(struct m_buf *mbuf)
{
                                 ...
                                 struct igb_lro_desc *lrod)
{
	...
	if (lrod->append_cnt) {
		...
		mbuf = igb_transform_rsc_queue(mbuf);
...
#endif
	igb_receive_mbuf(q_vector, mbuf, lrod->vlan_tag);
 ...
 **/
static u16 igb_lro_header_ok(struct m_buf *new_mbuf, struct iphdr *iph,
                               struct tcphdr *th)
{
...
/**
 * igb_lro_queue - if able, queue mbuf into lro chain
 * @q_vector: structure containing interrupt and ring information
 ...
 **/
static struct m_buf *igb_lro_queue(struct igb_q_vector *q_vector,
				       ...
				       u16 tag)
{
	...
	int tcp_data_len = igb_lro_header_ok(new_mbuf, iph, th);
	...
	hlist_for_each_entry(lrod, node, &lrolist->active, lro_node) {
		 ...
		 * size has changed, or new mbuf is larger than our current mss.
		...
		/* return the mbuf if it is empty for recycling */
		if (!new_mbuf->len) {
	...
	igb_receive_mbuf(q_vector, new_mbuf, tag);
	return NULL;
}
                                 ...
                                 int *work_done, int budget)
{
	...
	while (staterr & E1000_RXD_STAT_DD) {
		...
		mbuf = buffer_info->mbuf;
		prefetch(mbuf->data - NET_IP_ALIGN);
		buffer_info->mbuf = NULL;
		...
		if (buffer_info->dma) {
			...
			mbuf_put(mbuf, igb_get_hlen(rx_ring, rx_desc));
		}
		...
		if (length) {
			...
			mbuf->truesize += length;
		}

		if (!(staterr & E1000_RXD_STAT_EOP)) {
			buffer_info->mbuf = next_buffer->mbuf;
...
send_up:
		...
		if (staterr & (E1000_RXDADV_STAT_TSIP | E1000_RXDADV_STAT_TS))
			igb_rx_hwtstamp(q_vector, staterr, mbuf);
#endif
		total_bytes += mbuf->len;
		total_packets++;

		igb_rx_checksum_adv(rx_ring, staterr, mbuf);
		...
		if (igb_can_lro(rx_ring, rx_desc))
			buffer_info->mbuf = igb_lro_queue(q_vector, mbuf, vlan_tag);
		else
#endif
			igb_receive_mbuf(q_vector, mbuf, vlan_tag);
...
void igb_alloc_rx_buffers_adv(struct igb_ring *rx_ring, int cleaned_count)
{
	...
	while (cleaned_count--) {
		...
		if (!mbuf) {
			//printk("%s bufsz %d\n", __FUNCTION__, bufsz);
			mbuf = nta_alloc_mbuf(netdev, bufsz + NET_IP_ALIGN, 0);
			if (!mbuf) {
				rx_ring->rx_stats.alloc_failed++;
				goto no_buffers;
			}
			 ...
			 */
			mbuf_reserve(mbuf, NET_IP_ALIGN);

			buffer_info->mbuf = mbuf;
		}
		if (!buffer_info->dma) {
