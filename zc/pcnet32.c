...
struct pcnet32_private {
	...
	struct m_buf		**tx_skbuff;
	struct m_buf		**rx_skbuff;
...
static int pcnet32_start_xmit_fake(struct sk_buff *, struct net_device *);
static int pcnet32_start_xmit(struct m_buf *, struct net_device *);
				    ...
				    unsigned int size)
{
	...
	new_skb_list = kcalloc((1 << size), sizeof(struct m_buf *),
				GFP_ATOMIC);
	if (!new_skb_list) {
		if (netif_msg_drv(lp))
			printk("\n" KERN_ERR
				    ...
				    unsigned int size)
{
	...
	for (; new < size; new++ ) {
		struct m_buf *rx_skbuff;
		new_skb_list[new] = nta_alloc_mbuf(NULL, PKT_BUF_SKB, GFP_ATOMIC);
		...
		mbuf_reserve(rx_skbuff, NET_IP_ALIGN);
	...
	for (; new < lp->rx_ring_size; new++) {
		if (lp->rx_skbuff[new]) {
			pci_unmap_single(lp->pci_dev, lp->rx_dma_addr[new],
					 PKT_BUF_SIZE, PCI_DMA_FROMDEVICE);
			nta_kfree_mbuf(lp->rx_skbuff[new]);
		}
	}
	...
	for (; --new >= lp->rx_ring_size; ) {
		if (new_skb_list[new]) {
			pci_unmap_single(lp->pci_dev, new_dma_addr_list[new],
					 PKT_BUF_SIZE, PCI_DMA_FROMDEVICE);
			nta_kfree_mbuf(new_skb_list[new]);
		}
	}
	kfree(new_skb_list);
    free_new_lists:
...
static int pcnet32_loopback_test(struct net_device *dev, uint64_t * data1)
{
	...
	for (x = 0; x < numbuffs; x++) {
		if (!(skb = nta_alloc_mbuf(NULL, size, GFP_ATOMIC))) {
			if (netif_msg_hw(lp))
				printk(KERN_DEBUG
			...
			packet = skb->data;
			mbuf_put(skb, size);	/* create space for data */
			lp->tx_skbuff[x] = skb;
			     ...
			     int entry)
{
	...
	if (pkt_len > rx_copybreak) {
		...
		if ((newskb = nta_alloc_mbuf(NULL, PKT_BUF_SKB, GFP_ATOMIC))) {
			mbuf_reserve(newskb, NET_IP_ALIGN);
			skb = lp->rx_skbuff[entry];
			pci_unmap_single(lp->pci_dev,
					 lp->rx_dma_addr[entry],
					 PKT_BUF_SIZE,
					 PCI_DMA_FROMDEVICE);
			mbuf_put(skb, pkt_len);
		...
		skb = nta_alloc_mbuf(NULL, PKT_BUF_SKB, GFP_ATOMIC);
	}
	...
	skb->dev = dev;
	if (!rx_in_place) {
		mbuf_reserve(skb, NET_IP_ALIGN);
		mbuf_put(skb, pkt_len);	/* Make room */
		pci_dma_sync_single_for_cpu(lp->pci_dev,
					    lp->rx_dma_addr[entry],
					    pkt_len,
					    PCI_DMA_FROMDEVICE);
		mbuf_copy_to_linear_data(skb,
...
static int pcnet32_tx(struct net_device *dev)
{
	...
	while (dirty_tx != lp->cur_tx) {
		...
		if (lp->tx_skbuff[entry]) {
			...
			nta_kfree_mbuf(lp->tx_skbuff[entry]);
...
static const struct net_device_ops pcnet32_netdev_ops = {
	...
	.ndo_start_xmit		= pcnet32_start_xmit_fake,
...
pcnet32_probe1(unsigned long ioaddr, int shared, struct pci_dev *pdev)
{
	...
	lp->nta_index = nta_register_zc(dev, pcnet32_start_xmit);
...
static int pcnet32_alloc_ring(struct net_device *dev, const char *name)
{
	...
	lp->tx_skbuff = kcalloc(lp->tx_ring_size, sizeof(struct m_buf *),
				GFP_ATOMIC);
	if (!lp->tx_skbuff) {
		if (netif_msg_drv(lp))
			printk("\n" KERN_ERR PFX
	...
	lp->rx_skbuff = kcalloc(lp->rx_ring_size, sizeof(struct m_buf *),
				GFP_ATOMIC);
	if (!lp->rx_skbuff) {
		if (netif_msg_drv(lp))
			printk("\n" KERN_ERR PFX
...
static int pcnet32_init_ring(struct net_device *dev)
{
	...
	for (i = 0; i < lp->rx_ring_size; i++) {
		struct m_buf *rx_skbuff = lp->rx_skbuff[i];
		if (rx_skbuff == NULL) {
			if (!
			    (rx_skbuff = lp->rx_skbuff[i] =
			     nta_alloc_mbuf(NULL, PKT_BUF_SKB, GFP_ATOMIC))) {
...
static int pcnet32_start_xmit_fake(struct sk_buff *skb, struct net_device *dev)
{
	struct m_buf *mbuf = (struct m_buf*)skb;

	mbuf = nta_alloc_mbuf(NULL, PKT_BUF_SKB, GFP_ATOMIC);
	if(!mbuf) {
		kfree_skb(skb);
		return 0;
	}
	memcpy(mbuf->data, skb->data, PKT_BUF_SKB);
	mbuf->len = skb->len;
	mbuf->dir = NTA_DIR_TX;

	return pcnet32_start_xmit(mbuf, dev);
}

static int pcnet32_start_xmit(struct m_buf *skb, struct net_device *dev)
{
	struct pcnet32_private *lp = netdev_priv(dev);
	unsigned long ioaddr = dev->base_addr;
	u16 status;
