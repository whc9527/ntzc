                            ...
                            struct ethtool_eeprom *eeprom, u8 *bytes)
{
	...
	eeprom_buf = kmalloc(sizeof(u16) * eeprom_len, GFP_KERNEL);
	if (!eeprom_buf)
		return -ENOMEM;
	...
	for (i = 0; i < eeprom_len; i++)
		le16_to_cpus(&eeprom_buf[i]);

	memcpy(bytes, (u8 *)eeprom_buf + (eeprom->offset & 1), eeprom->len);
	kfree(eeprom_buf);
                            ...
                            struct ethtool_eeprom *eeprom, u8 *bytes)
{
	...
	first_word = eeprom->offset >> 1;
	last_word = (eeprom->offset + eeprom->len - 1) >> 1;
	eeprom_buf = kmalloc(max_len, GFP_KERNEL);
	if (!eeprom_buf)
		return -ENOMEM;
	...
	if (eeprom->offset & 1) {
		...
		ret_val = ixgbe_read_eeprom(hw, first_word, &eeprom_buf[0]);
		ptr++;
	}
	if (((eeprom->offset + eeprom->len) & 1) && (ret_val == 0)) {
		...
		ret_val = ixgbe_read_eeprom(hw, last_word,
		                  &eeprom_buf[last_word - first_word]);
	}
	...
	for (i = 0; i <= (last_word - first_word); i++)
		ret_val |= ixgbe_write_eeprom(hw, first_word + i, eeprom_buf[i]);
...
static int ixgbe_setup_desc_rings(struct ixgbe_adapter *adapter)
{
	...
	for (i = 0; i < tx_ring->count; i++) {
		...
		mbuf = nta_alloc_mbuf(NULL, size, GFP_KERNEL);
		if (!mbuf) {
			ret_val = 3;
			goto err_nomem;
		}
		...
		tx_ring->tx_buffer_info[i].length = mbuf->len;
		tx_ring->tx_buffer_info[i].dma =
			pci_map_single(pdev, mbuf->data, mbuf->len, PCI_DMA_TODEVICE);
		desc->read.buffer_addr = cpu_to_le64(tx_ring->tx_buffer_info[i].dma);
		desc->read.cmd_type_len = cpu_to_le32(mbuf->len);
		...
		case ixgbe_mac_82599EB:
			desc->read.olinfo_status |=
			                 (mbuf->len << IXGBE_ADVTXD_PAYLEN_SHIFT);
			break;
		default:
			break;
		}
...
#define IXGBE_MCSTCTRL_MO_MASK      0x00000003 /* Multicast Offset mask */
	...
	for (i = 0; i < rx_ring->count; i++) {
		...
		mbuf = nta_alloc_mbuf(NULL, IXGBE_RXBUFFER_2048 + NET_IP_ALIGN, GFP_KERNEL);
		if (!mbuf) {
			ret_val = 6;
			goto err_nomem;
		}
		mbuf_reserve(mbuf, NET_IP_ALIGN);
		rx_ring->rx_buffer_info[i].mbuf = mbuf;
		rx_ring->rx_buffer_info[i].dma =
			pci_map_single(pdev, mbuf->data, IXGBE_RXBUFFER_2048,
			               PCI_DMA_FROMDEVICE);
		rx_desc->read.pkt_addr =
				cpu_to_le64(rx_ring->rx_buffer_info[i].dma);
		memset(mbuf->data, 0x00, mbuf->len);
	}
...
static void ixgbe_create_lbtest_frame(struct m_buf *mbuf,
                                      unsigned int frame_size)
{
	memset(mbuf->data, 0xFF, frame_size);
	frame_size &= ~1;
	memset(&mbuf->data[frame_size / 2], 0xAA, frame_size / 2 - 1);
	memset(&mbuf->data[frame_size / 2 + 10], 0xBE, 1);
	memset(&mbuf->data[frame_size / 2 + 12], 0xAF, 1);
}

static int ixgbe_check_lbtest_frame(struct m_buf *mbuf,
                                    unsigned int frame_size)
{
	frame_size &= ~1;
	if (*(mbuf->data + 3) == 0xFF) {
		if ((*(mbuf->data + frame_size / 2 + 10) == 0xBE) &&
		    (*(mbuf->data + frame_size / 2 + 12) == 0xAF)) {
			return 0;
		}
	}
	return 13;
	...
	for (j = 0; j <= lc; j++) {
		...
		do {
			...
			ret_val = ixgbe_check_lbtest_frame(
					rx_ring->rx_buffer_info[l].mbuf, 1024);
			if (!ret_val)
				good_cnt++;
			if (++l == rx_ring->count)
				l = 0;
